...
export const categoryResolver = {
  Query: {
    categories: compose<any, ResolverContext>(authResolver, verifyTokenResolver)((parent, args, { entityAuthenticated, db }: ResolverContext, info: GraphQLResolveInfo) => db!.Category.findAll({
      where: {
  ...
  Mutation: {
    createCategory: compose<any, ResolverContext>(authResolver, verifyTokenResolver)((parent, { input }, { db, entityAuthenticated }: ResolverContext, info: GraphQLResolveInfo) => {
      ...
      if (loginType !== CollaboratorAccessTypeEnum.ADMIN) throw new Error('Just ADM can create a new category');
  ...
  Category: {
    restaurant: (category: CategoryAttributes, args, { dataLoaders: { restaurantLoader } }: { db: DbConnection; dataLoaders: DataLoaders }) => restaurantLoader.load(category.restaurantId!),
  },
};
