...
export const verifyTokenResolver: ComposableResolver<any, ResolverContext> = (resolver: GraphQLFieldResolver<any, ResolverContext>): GraphQLFieldResolver<any, ResolverContext> => (parent, args, context: ResolverContext, info) => {
  ...
  return jwt.verify(token!, JWT_SECRET!, async (err, decoded: any) => {
    if (!err) {
      const tokenInfo = <TokenInfo>decoded;
